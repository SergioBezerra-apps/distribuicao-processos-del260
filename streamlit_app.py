...
if all(k in files_dict for k in ["processos", "processosmanter", "observacoes", "disponibilidade"]):
    ...
    # =========================
    # Atribuições manuais por processo
    # =========================
    st.markdown("## Atribuições manuais por processo")
    st.caption(
        "Vincula um processo específico a um informante, independentemente de regras ou pools. "
        "Tem prioridade máxima e equivale a uma regra Exclusiva (Locked=True). "
        "O número do processo deve ser idêntico ao da planilha (incluindo formatação)."
    )

    if "manual_assignments" not in st.session_state:
        st.session_state["manual_assignments"] = pd.DataFrame(columns=["Processo", "Informante"])

    manual_assignments = st.data_editor(
        st.session_state["manual_assignments"],
        num_rows="dynamic",
        use_container_width=True,
        key="manual_assignments_editor",
        column_config={
            "Processo":   st.column_config.TextColumn("Processo (número exato)", required=True),
            "Informante": st.column_config.SelectboxColumn("Informante", options=inf_opts, required=True),
        }
    )
    st.session_state["manual_assignments"] = manual_assignments

    # Feedback: quantos processos manuais foram configurados
    n_manual = len(manual_assignments.dropna(subset=["Processo", "Informante"]))
    if n_manual > 0:
        st.info(f"{n_manual} atribuição(ões) manual(is) configurada(s). Esses processos serão destinados diretamente ao informante indicado.")
    ...
    if st.button("Executar Distribuição"):
        def run_distribution(processos_file, processosmanter_file, obs_file, numero,
            ...
            # 6) PRINCIPAIS: pool -> atribuições manuais -> regras -> redistribuição
            df_pool = res_df.copy().sort_values(by="Dias no Orgão", ascending=False).reset_index(drop=True)
            df_pool["Informante"] = ""
            if "Locked" not in df_pool.columns:
                df_pool["Locked"] = False

            # --- ATRIBUIÇÕES MANUAIS (prioridade máxima, Locked=True) ---
            assigned_manual = pd.DataFrame(columns=df_pool.columns)
            if manual_assignments_df is not None and not manual_assignments_df.empty:
                ma = manual_assignments_df.copy()
                ma["Processo"]   = ma["Processo"].astype(str).str.strip()
                ma["Informante"] = ma["Informante"].astype(str).str.strip().str.upper()
                ma = ma[
                    (ma["Processo"] != "") & (ma["Informante"] != "") & (ma["Informante"] != "NAN")
                ].drop_duplicates(subset=["Processo"])

                if not ma.empty:
                    manual_mask = df_pool["Processo"].isin(ma["Processo"])
                    n_encontrados = manual_mask.sum()
                    n_configurados = len(ma)
                    if n_encontrados < n_configurados:
                        processos_ausentes = set(ma["Processo"]) - set(df_pool[manual_mask]["Processo"])
                        st.warning(
                            f"Atribuições manuais: {n_encontrados}/{n_configurados} processo(s) encontrado(s) "
                            f"na planilha principal. Não encontrados: {sorted(processos_ausentes)}"
                        )

                    df_pool.loc[manual_mask, "Informante"] = df_pool.loc[manual_mask, "Processo"].map(
                        ma.set_index("Processo")["Informante"]
                    )
                    df_pool.loc[manual_mask, "Locked"] = True

                    assigned_manual = df_pool[manual_mask].copy()
                    df_pool = df_pool[~manual_mask].copy()
            ...
            res_final = pd.concat([assigned_manual, assigned_by_rules, distributed_df], ignore_index=True)
        ...
        try:
            (pre_geral_filename, pre_geral_bytes,
             ...
             informantes_emails, diag_df) = run_distribution(
                ...
                prev_file=prev_file,
                manual_assignments_df=st.session_state.get("manual_assignments")
            )
        except Exception as e:
    ...
    st.info("Carregue os quatro arquivos exigidos para habilitar a execução.")                      )        st.session_state.numero = numero else:    st.info("Carregue os quatro arquivos exigidos para habilitar a execução.")
