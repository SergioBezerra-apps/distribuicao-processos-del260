...
def run_distribution(processos_file, processosmanter_file, obs_file, disp_file, numero):
    ...
    # Retorna todos os dados necessários para o fluxo principal
    return (pre_geral_filename, pre_geral_bytes,
            res_geral_filename, res_geral_bytes,
            pre_df, res_assigned,  # <-- retorna também os DataFrames
            informantes_emails)
...
if "numero" not in st.session_state:
    st.session_state.numero = "184"  # valor padrão como string
...
if st.button("Executar Distribuição"):
    ...
    if all(key in files_dict for key in required_keys):
        ...
        (pre_geral_filename, pre_geral_bytes,
         res_geral_filename, res_geral_bytes,
         pre_df, res_assigned,  # agora retornando os DataFrames
         informantes_emails) = run_distribution(processos_file, processosmanter_file, obs_file, disp_file, numero)

        # ---------------------------------------------------------------------
        # Opção de filtro de Grupo Natureza para PRINCIPAIS (individual)
        # ---------------------------------------------------------------------
        ...
        for inf in informantes_principais:
            ...
            filtros_grupo_natureza[inf] = selected_grupos
        ...
        for inf in res_assigned["Informante"].dropna().unique():
            ...
            # FILTRO DE GRUPO NATUREZA PARA PRINCIPAIS
        ...
        # Atualiza o valor armazenado na sessão com o valor digitado
