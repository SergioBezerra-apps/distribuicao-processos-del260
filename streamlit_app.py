...
# ------------------------------------------
# Montagem dos filtros por informante
# ------------------------------------------
...
if all(key in files_dict for key in ["processos", "processosmanter", "observacoes", "disponibilidade"]):
    ...
    # Para simplificar, segue igual ao seu script original
    # Pré-atribuição
    ...
    # Distribuição (informantes_grupo_a e grupo_b)
    ...
    st.markdown("### Filtros de Grupo Natureza para Processos Principais (por informante)")
...
if st.button("Executar Distribuição"):
    ...
    if all(key in files_dict for key in required_keys):
        ...
        # ===================  REUTILIZA SUA LÓGICA ===========================
        # Os filtros_grupo_natureza já estão disponíveis no escopo global!

        def run_distribution(processos_file, processosmanter_file, obs_file, disp_file, numero, filtros_grupo_natureza):
        ...
        (pre_geral_filename, pre_geral_bytes,
         ...
         informantes_emails) = run_distribution(
            processos_file, processosmanter_file, obs_file, disp_file, numero, filtros_grupo_natureza
        )
        ...
        # Envio de e-mails (igual ao seu original)
        if not test_mode:
